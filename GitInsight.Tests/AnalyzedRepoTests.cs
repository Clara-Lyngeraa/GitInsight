...
//         //Creates 3 commits to the same date
    ...
    public async void check_if_datacommits_are_updated_in_datacommit_database_on_createAsync()
    {
        //Arrange
        var createDTO = new AnalyzedRepoCreateDTO(testRepo, testRepo.Info.Path);
        var (reponse, dto) = await _repo.CreateAsync(createDTO);

        //Act
        var dbRepo = _context.AnalyzedRepos.Find(dto.Id);
        ...
        public async void finding_a_repo_is_db_should_return_correct_analyzedRepo()
        {
            ...
            //Act
            var dbRepo = _context.AnalyzedRepos.Find(created.Id);
        ...
        public async void repo_path_with_new_commit_in_repo_should_be_the_same()
        {
            ...
            //Act
            var dbRepo = _context.AnalyzedRepos.Find(dbRepoFromCreate.Id);
        ...
        public async void Update_updates_list_of_datacommits_in_db()
        {
            //Arrange
            var createDTO = new AnalyzedRepoCreateDTO(testRepo, testRepo.Info.Path);
            var (response, dto) = await _repo.CreateAsync(createDTO);
